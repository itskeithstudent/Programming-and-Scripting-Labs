...
studentf_name = str(input("Enter a student first name please (BLANK to quit): ")).strip() # we will only exit if first name is blank could have individuals like Cher with no last name
...
if studentf_name == '': #if user has entered a '' we proceed no further
    ...
    while studentf_name !='': #while user hasn't entered a blank
        ...
        student_name_list = []
        studentl_name = str(input("Enter a student last name please: ")).strip()
        ...
        studentf_name = str(input("Enter a student first name please (BLANK to quit): ")).strip()
